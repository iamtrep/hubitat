...
metadata
{
    definition(
        ...
        command "getNeighbors", [[name: "Request the device's neighbor table via Mgmt_Lqi_req. Result logged at info level and stored in state.neighborTable."]]
        command "getRoutes", [[name: "Request the device's routing table via Mgmt_Rtg_req. Result logged at info level and stored in state.routeTable."]]
...
// On-device neighbor and routing tables, complementing the hub-side view from
// /hub/zigbee/getChildAndRouteInfo. Status text covers NOT_SUPPORTED so devices
// that lack ZDP routing-table support are easy to spot.
