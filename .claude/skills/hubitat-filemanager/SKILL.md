...
1. Check if `$ARGUMENTS` starts with `@hubname` (e.g., `@myhub upload myfile.html`). If so, use that hub name and strip the `@hubname` from arguments before further parsing. Otherwise, use `default_hub`.
