...
#
# test-well-monitor-app — Mode 1 behavior test
#
# Generated by /hubitat-behavior-test from apps/WellMonitor/tests/spec-well-monitor.yaml.
# Edit the spec and re-run the skill to regenerate this file. Manual edits
# will be overwritten.
#
# Usage:
#   bash apps/WellMonitor/tests/test-well-monitor-app.sh                             # default hub
#   bash apps/WellMonitor/tests/test-well-monitor-app.sh @hubname                    # specific hub
#   bash apps/WellMonitor/tests/test-well-monitor-app.sh @hubname 247                # specific hub + explicit instance id
#
# Runtime budget: ~45s. Tests exceeding this must opt into RUN_SLOW_TESTS=1 per TESTING.md §1.1.
#
...
APP_TYPE_NAME         = "Well Monitor"
APP_INSTANCE_LABEL    = "test-well-monitor-app"
MAKER_API_LABEL       = "test-well-monitor-maker"
INPUT_DEVICE_LABELS   = ["test-wm-pump", "test-wm-meter"]
OUTPUT_DEVICE_LABELS  = ["test-wm-pump-active", "test-wm-leak", "test-wm-flow-ind"]
CASES                 = [{'name': 'pump-start-turns-on-active-switch', 'setup': [{'device': 'test-wm-pump', 'command': 'setPower', 'args': [0]}, {'device': 'test-wm-meter', 'command': 'setVolume', 'args': [1000]}, {'device': 'test-wm-pump-active', 'command': 'off'}], 'setup_wait_seconds': 2, 'actions': [{'device': 'test-wm-pump', 'command': 'setPower', 'args': [200]}], 'wait_seconds': 3, 'assert': [{'device': 'test-wm-pump-active', 'attribute': 'switch', 'value': 'on'}], 'assert_logs': [{'pattern': 'Pump started', 'level': 'info'}]}, {'name': 'pump-stop-records-cycle', 'setup': [{'device': 'test-wm-meter', 'command': 'setVolume', 'args': [1007.5]}], 'setup_wait_seconds': 1, 'actions': [{'device': 'test-wm-pump', 'command': 'setPower', 'args': [0]}], 'wait_seconds': 3, 'assert': [{'device': 'test-wm-pump-active', 'attribute': 'switch', 'value': 'off'}], 'assert_logs': [{'pattern': 'Pump stopped', 'level': 'info'}, {'pattern': 'coincident flow 7\\.5', 'level': 'info'}]}, {'name': 'flow-start-turns-on-flow-indicator', 'setup': [{'device': 'test-wm-meter', 'command': 'setRate', 'args': [0]}, {'device': 'test-wm-flow-ind', 'command': 'off'}], 'setup_wait_seconds': 2, 'actions': [{'device': 'test-wm-meter', 'command': 'setRate', 'args': [5]}], 'wait_seconds': 3, 'assert': [{'device': 'test-wm-flow-ind', 'attribute': 'switch', 'value': 'on'}], 'assert_logs': [{'pattern': 'Water flow started', 'level': 'info'}]}, {'name': 'flow-stop-records-event', 'setup': [{'device': 'test-wm-meter', 'command': 'setVolume', 'args': [1010.5]}], 'setup_wait_seconds': 1, 'actions': [{'device': 'test-wm-meter', 'command': 'setRate', 'args': [0]}], 'wait_seconds': 3, 'assert': [{'device': 'test-wm-flow-ind', 'attribute': 'switch', 'value': 'off'}], 'assert_logs': [{'pattern': 'Water flow stopped', 'level': 'info'}]}, {'name': 'emergency-shutoff-fires-after-max-runtime', 'setup': [{'device': 'test-wm-pump', 'command': 'setPower', 'args': [0]}, {'device': 'test-wm-leak', 'command': 'dry'}, {'device': 'test-wm-pump-active', 'command': 'off'}], 'setup_wait_seconds': 2, 'actions': [{'device': 'test-wm-pump', 'command': 'setPower', 'args': [200]}], 'wait_seconds': 14, 'assert': [{'device': 'test-wm-leak', 'attribute': 'water', 'value': 'wet'}, {'device': 'test-wm-pump-active', 'attribute': 'switch', 'value': 'off'}], 'assert_events': [{'attribute': 'water', 'value': 'wet', 'source': 'test-wm-leak'}, {'attribute': 'switch', 'value': 'off', 'source': 'test-wm-pump-active'}], 'allow_log_patterns': ['No notification device configured']}]
