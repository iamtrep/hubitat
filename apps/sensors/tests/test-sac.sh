#!/usr/bin/env bash
#
# test-sac-app — Mode 1 behavior test
#
# Generated by /hubitat-behavior-test from apps/sensors/tests/spec-sac.yaml.
# Edit the spec and re-run the skill to regenerate this file. Manual edits
# will be overwritten.
#
# Usage:
#   bash apps/sensors/tests/test-sac.sh                             # default hub
#   bash apps/sensors/tests/test-sac.sh @hubname                    # specific hub
#   bash apps/sensors/tests/test-sac.sh @hubname 247                # specific hub + explicit instance id
#
...
APP_TYPE_NAME         = "Sensor Aggregator Child"
APP_INSTANCE_LABEL    = "test-sac-app"
MAKER_API_LABEL       = "test-sac-maker"
INPUT_DEVICE_LABELS   = ["test-sac-in-1", "test-sac-in-2", "test-sac-in-3"]
OUTPUT_DEVICE_LABELS  = ["test-sac-out"]
CASES                 = [{'name': 'all-equal-baseline', 'setup': [{'device': 'test-sac-in-1', 'command': 'setTemperature', 'args': [20.0]}, {'device': 'test-sac-in-2', 'command': 'setTemperature', 'args': [20.0]}, {'device': 'test-sac-in-3', 'command': 'setTemperature', 'args': [20.0]}], 'wait_seconds': 2, 'assert': [{'device': 'test-sac-out', 'attribute': 'temperature', 'value': 20.0, 'tolerance': 0.05}]}, {'name': 'single-input-raised', 'actions': [{'device': 'test-sac-in-1', 'command': 'setTemperature', 'args': [30.0]}], 'wait_seconds': 2, 'assert': [{'device': 'test-sac-out', 'attribute': 'temperature', 'value': 23.333, 'tolerance': 0.1}]}, {'name': 'idempotent-reset', 'actions': [{'device': 'test-sac-in-1', 'command': 'setTemperature', 'args': [20.0]}], 'wait_seconds': 2, 'assert': [{'device': 'test-sac-out', 'attribute': 'temperature', 'value': 20.0, 'tolerance': 0.05}]}]
