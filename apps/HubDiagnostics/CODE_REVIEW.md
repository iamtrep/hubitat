...
**Net new actionable from Round 9: 14 (R9-1 … R9-14), none fixed.** Highest value: the dead-code deletions (R9-1/2/3 — ~80 lines, zero behavior change) and the badge/timing/CSV extractions (R9-4/5/6/8).
...
| R9-1 | Dead methods ×5 (definition-only): `buildRadioProtocolMap`, `listHubFilesByNames`, `formatDuration`, `formatDurationSec`, `fileExists` (~49 lines) | ⚪ | **Net-new** — grep-confirmed dead; safe delete (`formatDuration` also drops C7) | [ ] |
| R9-2 | Dead Groovy symbols ×4: `RECOMMENDED_ZIGBEE_CHANNELS`, `ZIGBEE_LQI_WARN`, `ZWAVE_PER_CRIT`, `byStatus` key (`getEmptyDeviceStats`) | ⚪ | **Net-new** — referenced only at declaration | [ ] |
| R9-3 | Dead SPA symbols ×3: `buildParentTypeMap`, `PD` const, `.audit-body`/`#audit-root body` CSS | ⚪ | **Net-new** — definition-only | [ ] |
| R9-4 | SPA badge markup duplicated ×5 (`srcBadge`/`perfSrcBadge`/`driverBadge` + inline ternary) | 🟡 | **Net-new** — extract one `srcBadge(source, opts)` | [ ] |
| R9-5 | Two SPA-internal sort engines: `tbl()` and `initAuditSort()` (audit HTML is string-built) | 🟠 | **Net-new (continues #3)** — render audit via `tbl()` | [ ] |
| R9-6 | Per-endpoint API-timing boilerplate ×8 | 🟡 | **Net-new** — extract `timed(name, Closure)` | [ ] |
| R9-7 | Duplicate `/device/fullJson/` constant: `FULL_JSON_PATH_PREFIX` + `DEVICE_FULL_JSON_PATH` | ⚪ | **Net-new** — collapse to one | [ ] |
| R9-8 | CSV row-extraction duplicated: `fetchSystemResources` / `fetchMemoryHistory` | 🟡 | **Net-new** — `parseResourceRow(row)` | [ ] |
| R9-9 | Groovy duplication cluster: `.with{ok?data}` ×9; ZW problem-node predicate ×3; `allRadioDevices` join ×2; `controllerType` conn/label tables; `isVersionAtLeast`/`isNewer` | ⚪ | **Net-new** | [ ] |
| R9-10 | SPA duplication cluster: snap-table row-map ×2; 3 live-res renderers; ghost/failed split ×2; `renderDiff` inline style ×25; `fmem`/`fbytes` twins | ⚪ | **Net-new** | [ ] |
