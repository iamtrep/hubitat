...
Combined output of independent passes:
...
- **Claude round 7** — hub→browser load-balancing audit (5 deferred items LB1–LB5; Tier-1 derivations moved to SPA)
- **Claude round 8** — doc reconciliation audit against v5.38.0 (4 items resolved by removal, N8 fixed, 1 new fixed item R8-1, C1 relocated; see Round 8 summary)
- **Claude round 9** — dead-code / duplication / bloat sweep + arch-compliance audit against v5.38.0 (14 net-new items R9-1–R9-14; 0 arch errors; bloat reconciled against #2/C13)
...
## Round 8 summary (Claude, 2026-05-21, v5.38.0 — doc reconciliation audit)

Full audit of this document against the current code, triggered by drift: the header still said "post-v5.27.0", but the Round 7 refactors (v5.25.0–v5.32.x) plus later work (v5.33.0 checkpoint-index split, v5.37.0 device inventory + finalize-race fix, v5.38.0 alert unification) had silently obsoleted or relocated items still marked Open. **The findings table below and this summary are now authoritative; the *Detailed findings* prose predates these refactors and its line numbers are approximate.**

**Resolved by removal** (the code the finding describes no longer exists):
- **#2 / #3** — `renderAuditHtml`, the `AUDIT_REPORT_CSS` `@Field`, and `apiForumExport` are gone; audit/forum rendering moved entirely to the SPA (`buildAuditHtml`, `buildForumMarkdown`) in v5.25.0. The "Groovy audit duplicates the SPA framework" concern is structurally resolved.
- **C4** — `apiGenerateReport` deleted (v5.32.0), replaced by `apiReportTemplate()` + thin `apiSaveReport()`. No server-side `/hub2/appsList` multi-fetch remains.
- **C5** — alert composition moved to the SPA's `composeAlerts()`; `getStructuredAlerts` → `getAlertSignals` (v5.30.0), finalized v5.38.0 to ship only raw signals. `analyzeSystemHealth` explicitly no longer composes alerts.
- **A2 / `apiSnapshotDiff`** — `apiSnapshotDiff`, `/api/snapshot/diff`, and `saveSnapshotDiffPayload` all deleted (v5.31.0); diffing moved to SPA `computeSnapshotDiff`.
...
- **N4** — `cachedCheckpoints` was replaced by split-file `cachedCheckpointIndex` + per-checkpoint detail files (v5.33.0). The no-defensive-copy concern persists: `loadCheckpointIndex()` returns the shared reference; `saveCheckpointIndex()` stores the caller's.
- **C6** — partial: `readFile()` still returns `def` and `buildZwaveGhostNodes` still has a `def` local; the `apiSnapshotDiff` `def` locals are gone with the method.
- **LB2** — `analyzeNetwork` is now a thin raw aggregator (no hub-side sorts); the `analyzeApps` type/name sorts persist (SPA re-sorts them anyway).
- **LB4** — Z-Wave ghost recount is still synchronous (≤8s) inside `getAlertSignals`, now gated by a 60s `state.lastZwaveGhostCheckMs` window with `state.cachedZwaveSignals` fallback. Not moved to a scheduler; there is no `state.cachedZwaveGhostCount`.
- **A9** — `extractAuditFields` `_schemaVersion` is now `2` (bumped by the v5.37.0 device-inventory fields).

**N1 scope grew.** The original radio/list caches still aren't cleared in `updated()`; `cachedCheckpoints` became `cachedCheckpointIndex` (also uncleared); and v5.38.0 added more uncleared volatile caches: `cachedSystemResources/At`, `cachedTemperature/At`, `cachedDatabaseSize/At`, `cachedCpuInfo/At`, `cachedLoadThreshold/At`. `updated()` still clears only `zwaveStackCache`, `fwUpdateCache/At`, `state.controllerTypeCache`, `apiTimings`.

**Still open, re-verified against v5.38.0** (current refs): #5 (`finalizeAudit:4385/4398`), #8, C2 (no `logsOff`), C3 (`generateQuickSummary:3497`, called from prefs page `:415`, uncached), C7 (only `stripHtml` has `@CompileStatic`), C8 (`fetchHubMessages` uncached in `getAlertSignals:1473`), C9 (`*/N` cron `:4704`), C10 (`autoEnableOAuth` raw httpGet/Post `:3703`), C11 (regex HTML parsing — `fetchZwaveTopology:2184`, `fetchZigbeeMeshInfo:2236`), C12, C13 (`buildForumMarkdown` ~297 / `buildCrossReference:4118` ~120 lines), C15 (`lastAuditResult:90`), N2 (~19 call sites, no `cachedFetch` helper), N3 (4× inline in `getPerformanceData`), N5 (`apiCode:646`, `apiPerformance:708` still uncommented), N7 (SPA `api()` has no in-flight Promise registry), LB1 (`buildCrossReference` still hub-side), LB3 (`apiForumData:971` — 13 serial calls), LB5 (`enrichDevices:3007` unbounded fan-out). Deferrals unchanged: B1, A5, G2, G3.

**Net new actionable from Round 8: one (R8-1, already fixed).** The main outcome is reconciliation — 4 items resolved by removal, N8 fixed, several relocated/clarified. C1 stays the only critical open item.
...
- **R9-2** — four dead Groovy symbols: `RECOMMENDED_ZIGBEE_CHANNELS` (`:93`), `ZIGBEE_LQI_WARN` (`:129`, only `ZIGBEE_LQI_CRIT` is used), `ZWAVE_PER_CRIT` (`:131`, the PER>1 checks inline a literal `1`), and the `byStatus` key seeded in `getEmptyDeviceStats` (`:3588`, never read — `idsByStatus` is the populated one).
- **R9-3** — three dead SPA symbols: `buildParentTypeMap` (`:822`, superseded by hub-shipped `appParentTypeById`), the `PD` protocol-display const (`:278`), and the `.audit-body` / `#audit-root body` CSS rule (`:100` — `#audit-root` is a `<div>`, so the selector can never match).

**Duplication (net-new):**
- **R9-5 (🟠, continues #3)** — two table sort/filter engines now coexist *inside the SPA*: `tbl()` (`:860`) and `initAuditSort()` (`:3395`). #3 removed the Groovy↔SPA copy; this is the re-emergence inside the SPA because audit HTML is server-string-built rather than data-driven. Long-term fix: render audit tables through `tbl()`.
- **R9-4 (🟡)** — source/driver badge markup duplicated ×5 (`srcBadge` `:1313`/`:1392`, `perfSrcBadge` `:1993`, `driverBadge` `:3129`, inline ternary `:3352`). Extract one `srcBadge(source, {emptyAsBuiltin})`.
- **R9-6 (🟡)** — per-endpoint API-timing boilerplate ×8 (`:598/627/637/646/662/676/708/717`): identical `start`/`elapsed`/`logDebug`/`recordApiTiming`/`return`. Extract `Map timed(String name, Closure<Map> body)`.
- **R9-8 (🟡)** — CSV row-extraction duplicated between `fetchSystemResources` (`:1727`) and `fetchMemoryHistory` (`:1760`); a `parseResourceRow(Map)` helper also shrinks exposure to the known CSV column-drift problem.
- **R9-7 (⚪)** — duplicate `/device/fullJson/` constant: `FULL_JSON_PATH_PREFIX` (`:77`) and `DEVICE_FULL_JSON_PATH` (`:265`).
- **R9-9 (⚪, Groovy cluster)** — `.with { it.ok ? it.data : null }` ×9 (3 disappear with R9-1's dead `buildRadioProtocolMap`); Z-Wave problem-node predicate ×3 (`:1188/1191/1534`, would consume the dead `ZWAVE_PER_CRIT`); `allRadioDevices` join ×2 (`:1002/1332`); `controllerType` conn-table (`:270`) vs label-switch (`:4113`); `isVersionAtLeast` (`:1654`) / `isNewer` (`:3677`).
- **R9-10 (⚪, SPA cluster)** — snapshot-table row-map ×2 (`:2108/2154`); three live-resource renderers share a prefix (`:1050/1057/1071`); ghost/failed split ×2 (`:551/1608`); `renderDiff` inline-style string ×25 → a `.diff-item` class; `fmem`/`fbytes` twin formatters (`:268/269`).

**Arch (net-new warnings/notes):**
- **R9-11 (🟡, extends C2)** — logging discipline incomplete: only a `debugLogging` pref. ARCHITECTURE *Logging discipline* expects three prefs (add `txtEnable`/`traceEnable`) plus gated `logTrace/logDebug/logInfo/logWarn/logError` helpers. C2 covers only the missing `logsOff`; the missing trace/desc prefs + `logTrace` helper are net-new.
- **R9-12 (⚪, cross-ref G1)** — `state.audit` is written from the async callback `fullJsonCb` (`:4340`) and from `finalizeAudit` reached inline from it (`:4443`) using plain `state`, not `atomicState` (ARCHITECTURE *Patterns To Avoid*). **Tempered:** G1 already analyzed this race — snapshots only lag, never lead, the terminal value is always correct, and finalize fires only when `inFlight==0` (no concurrent writer). Doc-compliance flag, not a bug.
- **R9-13 (⚪, extends C6)** — new `def` locals where a concrete type fits: `prevNewestMs`/`newestMs` (`:899/902`, Long), `numericId = app.id` (`:2648`), `deviceId` (`:2852`).
- **R9-14 (⚪, judgment calls)** — `fetchZwaveDataForCheckpoint`/`fetchZigbeeDataForCheckpoint` twins (`:3273/3284`) → parameterize like `chainNextRadio(boolean)`; `_schemaVersion:2` sentinel (`:4046`) is defensive for in-memory-only `AUDIT_SCANS` (cross-ref A9 — keep only if on-disk persistence is planned); `renderComparison` manual-build branch (`:2065-2076`) is likely unreachable under the current hub payload — **verify before removing**; `getAppTypeId` raw `httpGet` (`:3697`) re-fetches what `fetchUserAppTypes` already wraps (cross-ref C10); `fullJsonCb` (`:4315`) checks `resp?.status` without `resp.hasError()` first (fail-soft).

**Bloat — reconciled, no new rows.** Long methods flagged in the quality pass (`getPerformanceData` ~178, `rPerf` ~217, `computeSnapshotDiff` ~135, `buildAuditHtml` ~291, `buildForumMarkdown` ~296, `analyzeDevices` ~176, `buildCrossReference` ~119) all map to the existing **#2** "accepted as coherent pipelines" stance, **C13** (known complexity), and **LB1**. Round 9 reaffirms that stance: the worthwhile structural work is the *true duplication* (R9-4/5/6/8), not raw length. `MOCK_DATA` shipped in production → **#11** ("Overstated — accept").

**Net new actionable from Round 9: 14 (R9-1 … R9-14).** Shipped in **v5.40.0**: R9-1/2/3/4/6/7/8 (full) and R9-9/R9-10 (partial — see notes), a behavior-preserving cleanup of **net −85 lines** verified by compile + the 191-test API suite. **R9-5 deferred** (large audit-render rewrite). Within R9-9/R9-10 two sub-items were deliberately **not** consolidated to avoid behavior changes: the `isVersionAtLeast`/`isNewer` merge (different `>=`/`>` + edge-case semantics) and the `fmem`/`fbytes` merge (different scale/tiers/precision). Still open: R9-11 (logging prefs), R9-12 (async `state.audit`), R9-13 (`def` locals), R9-14 (notes).
...
| 2 | Oversized methods (`renderAuditHtml` 363 lines, `apiSnapshotDiff` 242, `analyzeDevices` 172) | 🟡 | **Resolved by removal (R-8)** — `renderAuditHtml`/`apiSnapshotDiff`/`apiForumExport` deleted (rendering moved to SPA v5.25.0/v5.31.0); only `analyzeDevices` (177) + `analyzeApps` (155) remain, accepted as coherent pipelines | [x] |
| 3 | Audit report duplicates SPA framework (own CSS / own table-sort JS) | 🟠 | **Resolved v5.25.0 (R-8)** — audit rendering moved entirely to SPA `buildAuditHtml`; Groovy `renderAuditHtml` + `AUDIT_REPORT_CSS` deleted | [x] |
...
| A9 | `extractAuditFields` schema has no version sentinel; pre/post-schema records indistinguishable | ⚪ | **Fixed v5.18.0** (`_schemaVersion`; bumped to 2 in v5.37.0) | [x] |
...
| C1 | Silent `createSnapshot()` failure → wrong diff. `apiSnapshotDiff` deleted v5.31.0 but risk **relocated (R-8)**: `apiCreateSnapshot` always returns success; SPA create-then-diff trusts index 0 | 🔴 | Open (relocated) | [ ] |
...
| C4 | `apiGenerateReport` fetches `/hub2/appsList` 3–4× in one call (shared cache gap) | 🟡 | **Obsolete v5.32.0 (R-8)** — replaced by `apiReportTemplate`+`apiSaveReport`; no server-side multi-fetch | [x] |
| C5 | Alert threshold logic duplicated between `getStructuredAlerts` and `analyzeSystemHealth` | 🟡 | **Obsolete v5.30.0/v5.38.0 (R-8)** — composition moved to SPA `composeAlerts`; hub ships raw signals only | [x] |
| C6 | `readFile()` return untyped `def`; stray `def` locals | ⚪ | Open (partial, R-8) — `readFile`/`buildZwaveGhostNodes` `def` remain; `apiSnapshotDiff` locals gone with the method | [ ] |
| C7 | Pure-computation methods missing `@CompileStatic` (only `stripHtml` has it) | ⚪ | Open | [ ] |
| C8 | `fetchHubMessages()` called unconditionally in alert hot path (`getAlertSignals`) with 5s timeout, no cache | ⚪ | Open | [ ] |
...
| N2 | Caching strategy fragmented: same hub endpoints fetched at 11+ call sites; only `getPerformanceData` uses the new TTL caches | 🟠 | **Fixed** (v5.83.6): `cachedFetch()` (`fb6e35c`) is used by the perf path, the async radio chain and, since v5.83.6, `analyzeNetwork`. Remaining direct fetches are deliberate: config snapshot and audit read fresh for a point-in-time record, `analyzeDevices`/`buildMeshFieldsMap` take prefetched data, and the apps-list fetches in `analyzeApps`/`buildAppLookupMap` are cheap enough to leave | [x] |
...
| N8 | Version drift in WIP: `APP_VERSION=5.29.0` vs `UI_VERSION=5.28.0` | ⚪ | **Fixed** — both `5.38.0` (R-8 verified) | [x] |
...
| LB2 | Per-tab sorts done hub-side; SPA `tbl()` re-sorts on column click anyway | ⚪ | Open (Tier 2, partial R-8) — `analyzeNetwork` no longer sorts (thin aggregator); `analyzeApps` type/name sorts persist | [ ] |
...
| LB4 | Z-Wave ghost recount synchronous (≤8s) inside `getAlertSignals()` hot path; now 60s state-gated (R-8) but not moved to a scheduler | 🟡 | Open (Tier 3) | [ ] |
...
| R8-1 | Audit finalize never triggers when the `inFlight`-zeroing callback's local `processed` is stale → scan stuck until the watchdog fails it | 🔴 | **Fixed v5.37.0** (finalize gate reads `processed` fresh from the atomic) | [x] |
| R9-1 | Dead methods ×5 (definition-only): `buildRadioProtocolMap`, `listHubFilesByNames`, `formatDuration`, `formatDurationSec`, `fileExists` (~49 lines) | ⚪ | **Fixed v5.40.0** — all five deleted (also closes the `formatDuration` row of C7) | [x] |
| R9-2 | Dead Groovy symbols ×4: `RECOMMENDED_ZIGBEE_CHANNELS`, `ZIGBEE_LQI_WARN`, `ZWAVE_PER_CRIT`, `byStatus` key (`getEmptyDeviceStats`) | ⚪ | **Fixed v5.40.0** — deleted `RECOMMENDED_ZIGBEE_CHANNELS`, `ZIGBEE_LQI_WARN`, `byStatus`; `ZWAVE_PER_CRIT` **kept** (now consumed by the R9-9 predicate) | [x] |
| R9-3 | Dead SPA symbols ×3: `buildParentTypeMap`, `PD` const, `.audit-body`/`#audit-root body` CSS | ⚪ | **Fixed v5.40.0** — all three deleted | [x] |
| R9-4 | SPA badge markup duplicated ×5 (`srcBadge`/`perfSrcBadge`/`driverBadge` + inline ternary) | 🟡 | **Fixed v5.40.0** — shared `BADGE` markup map + `srcBadge(source,emptyAsBuiltin)` (rDevices/rApps/rPerf) + `driverBadge` (audit ×2) | [x] |
| R9-5 | Two SPA-internal sort engines: `tbl()` and `initAuditSort()` (audit HTML is string-built) | 🟠 | **Deferred** — ~300-line `buildAuditHtml` rewrite with manual SPA verification; out of scope for the v5.40.0 cleanup batch | [-] |
| R9-6 | Per-endpoint API-timing boilerplate ×8 | 🟡 | **Fixed v5.40.0** — `timed(name, Closure)` wraps the 7 identical aggregators; `apiZigbeeScan` left (distinct return shape) | [x] |
| R9-7 | Duplicate `/device/fullJson/` constant: `FULL_JSON_PATH_PREFIX` + `DEVICE_FULL_JSON_PATH` | ⚪ | **Fixed v5.40.0** — collapsed to `FULL_JSON_PATH_PREFIX` | [x] |
| R9-8 | CSV row-extraction duplicated: `fetchSystemResources` / `fetchMemoryHistory` | 🟡 | **Fixed v5.40.0** — `parseResourceRow(row)`; each caller keeps its own keys + missing-column control flow | [x] |
| R9-9 | Groovy duplication cluster: `.with{ok?data}` ×9; ZW problem-node predicate ×3; `allRadioDevices` join ×2; `controllerType` conn/label tables; `isVersionAtLeast`/`isNewer` | ⚪ | **Fixed v5.40.0 (partial)** — `reqData`, `buildRadioDeviceList`, `isZwaveProblemNode` (consumes `ZWAVE_PER_CRIT`) shipped + `controllerType` cross-ref comment. ZW predicate consolidated at 2 of 3 sites; `computeZwaveSignals` left (its `(per as int)` truncation differs). `isVersionAtLeast`/`isNewer` **not** merged (different `>=`/`>` + split/tokenize edge cases) | [x] |
| R9-10 | SPA duplication cluster: snap-table row-map ×2; 3 live-res renderers; ghost/failed split ×2; `renderDiff` inline style ×25; `fmem`/`fbytes` twins | ⚪ | **Fixed v5.40.0 (partial)** — `renderSnapTable`, `resMemCard` (the one identical card; CPU/temp/db cards legitimately differ), `splitGhostNodes`, `.diff-item` class (30 inline styles removed) shipped. `fmem`/`fbytes` **not** merged (different scale/tiers/precision) | [x] |
| R9-11 | Logging discipline: only `debugLogging` pref; no `txtEnable`/`traceEnable` + `logTrace` helper | 🟡 | **Net-new (arch warning)** — extends C2 | [ ] |
| R9-12 | `state.audit` written from async callback (`fullJsonCb`/`finalizeAudit`) with plain `state` not `atomicState` | ⚪ | **Net-new (arch note)** — but G1 shows lag-only, terminal value correct | [ ] |
| R9-13 | `def` locals where concrete type fits (`:899/902/2648/2852`) | ⚪ | **Extends C6** — new instances | [ ] |
| R9-14 | Notes: `fetchZwave/ZigbeeDataForCheckpoint` twins; `_schemaVersion` sentinel (A9); `renderComparison` dead branch (verify payload); `getAppTypeId` raw `httpGet` (C10); `fullJsonCb` missing `hasError()` | ⚪ | **Notes / judgment calls** | [ ] |
...
> **[R-8, v5.38.0]:** `renderAuditHtml`, `apiSnapshotDiff`, and `apiForumExport` have since been deleted (rendering moved to the SPA). Of the list below only `analyzeDevices` (~177) and `analyzeApps` (~155) remain — both accepted as coherent pipelines.
...
> **[R-8, v5.38.0]:** Several backlog items below are now resolved or obsolete — see the Round 8 summary. Reconciliation: **C4, C5 obsolete** (code removed); **N8 done** (versions synced at 5.38.0); **R8-1 fixed** (finalize race); **C1 relocated** to `apiCreateSnapshot`. The live open backlog is the "Still open" list in the Round 8 summary; the phase checklists below are historical record.
...
## Round 9 detailed findings (Claude, 2026-05-21, v5.38.0)

First dedicated dead-code sweep plus a fresh duplication/arch pass. Bloat findings were reconciled against the existing #2/C13 "accepted coherent pipeline" stance and produced no new rows. Each dead claim was grep-verified (the symbol appears exactly once = at its declaration).

### ⚪ R9-1 — five never-called Groovy methods

**Where (verified definition-only):**

| Method | Line | Notes |
|---|---|---|
| `buildRadioProtocolMap()` | 2879 | builds an id→protocol map (3 hub HTTP calls) nothing consumes; deleting it also removes 3 of R9-9's `.with{}` sites |
| `listHubFilesByNames(List)` | 3499 | no caller |
| `formatDuration(int)` | 3657 | the audit path uses `formatDurationSec`; **C7** recommends `@CompileStatic` here — moot, delete instead |
| `formatDurationSec(Long)` | 4272 | no caller (was it meant to be wired into the audit report?) |
| `fileExists(String)` | 3986 | no caller |

**Fix:** delete all five (~49 lines). Zero behavior change.

### ⚪ R9-2 — four dead Groovy symbols

`RECOMMENDED_ZIGBEE_CHANNELS` (`:93`) and `ZIGBEE_LQI_WARN` (`:129`) are declared but never referenced (only `ZIGBEE_LQI_CRIT` is used, `:2295`). `ZWAVE_PER_CRIT` (`:131`) is unused because the three PER>1 checks inline a literal `1` (see R9-9). `getEmptyDeviceStats` (`:3588`) seeds `byStatus: [active:0, inactive:0, disabled:0]`, but `analyzeDevices` only ever populates `idsByStatus` — `byStatus` is never read. Delete the four; or, for `ZWAVE_PER_CRIT`, wire it into the R9-9 predicate instead.

### ⚪ R9-3 — three dead SPA symbols

`buildParentTypeMap` (`:822`) was superseded by the hub-shipped `appParentTypeById` (the Performance CPU-by-app-type chart now uses that). `PD` (`:278`, protocol-display map) is never referenced — its sibling `CD` (`:277`) is used heavily. `#audit-root body,.audit-body{padding:16px}` (`:100`) can never match: `.audit-body` is set on no element and `#audit-root` is a `<div>` that cannot contain a `<body>` (it uses an inline `padding` instead). Delete the function, the const, and the CSS rule.

### 🟠 R9-5 — two table sort/filter engines inside the SPA (continues #3)

**Where:** `tbl()` (`:860`, the data-driven filter/sort/debounce engine) and `initAuditSort()` (`:3395`, a parallel sort+filter engine for `table[data-sortable]` in audit mode). Both implement header-click toggling, numeric-vs-string compare, arrow indicators, and a text filter. #3 ("audit duplicates the SPA framework") was resolved by moving audit rendering *into* the SPA — but because audit HTML is built as a server-style string rather than driven by row data, it grew its own second engine. **Fix (longer-term):** render audit tables through `tbl()` so there is one engine; this also unlocks deleting `initAuditSort` and the `data-sortable` scaffolding.

### 🟡 R9-4 — source/driver badge markup duplicated ×5

**Where:** `srcBadge` in `rDevices` (`:1313`, keyed `builtin`/`community`) and `rApps` (`:1392`, `platform`/`community`/else), `perfSrcBadge` (`:1993`, byte-identical to `:1392` except an empty-source fallback), `driverBadge` in audit (`:3129`), and an inline ternary (`:3352`). All emit the same `<span class="badge b-*">` markup. **Fix:** one `srcBadge(source, {emptyAsBuiltin})` near the other formatters (`~:271`); removes ~15 lines and a class of drift bugs.

### 🟡 R9-6 — per-endpoint API-timing boilerplate ×8

**Where:** `apiDashboard` (`:598`), `apiDevices` (`:627`), `apiApps` (`:637`), `apiCode` (`:646`), `apiNetwork` (`:662`), `apiHealth` (`:676`), `apiPerformance` (`:708`), `apiZigbeeScan` (`:717`). Each is the identical `long start = now() … long elapsed = now()-start; logDebug "X completed in ${elapsed}ms"; recordApiTiming("X", elapsed); return jsonResponse(data)`. **Fix:** `Map timed(String name, Closure<Map> body)` wrapper.

### 🟡 R9-8 — CSV row-extraction duplicated

**Where:** `fetchSystemResources` (`:1727`) and `fetchMemoryHistory` (`:1760`) both run the same `csvField(row, ["Free OS", "Free OS Memory"])` / `["5m CPU avg", …]` / Free-Java / Direct-Java alias lookups, missing-column warn, and `.toInteger()`/`.toFloat()` parsing. **Fix:** a shared `Map parseResourceRow(Map row)` — also narrows the surface for the known CSV column-drift issue.

### 🟡 R9-11 — logging discipline incomplete (extends C2)

**Where:** the only log pref is `debugLogging` (`:481`); the gated helpers (`:3541-3555`) are `logDebug/logInfo/logWarn/logError` with no `logTrace`, and there is no `txtEnable` (descriptionText) or `traceEnable` pref. ARCHITECTURE *Logging discipline* expects three boolean prefs plus the small set of gated helpers including `logTrace`. **C2** already tracks the missing `logsOff` auto-disable; the missing prefs + `logTrace` helper are the net-new part.

### ⚪ R9-7, R9-9, R9-10, R9-12, R9-13, R9-14 — clustered low-severity items

See the Round 9 summary above for per-item lines and rationale. R9-7/9 are Groovy DRY consolidations (duplicate `/device/fullJson/` constant; `.with{ok?data}` ×9; Z-Wave problem-node predicate ×3 that should consume the dead `ZWAVE_PER_CRIT`; `allRadioDevices` join ×2; `controllerType` conn-table vs label-switch; `isVersionAtLeast`/`isNewer`). R9-10 is the SPA equivalent (snapshot row-map ×2; three live-resource renderers; ghost/failed split ×2; `renderDiff` inline style ×25 → a `.diff-item` class; `fmem`/`fbytes` twins). R9-12 (async `state.audit` write) is a doc-compliance flag tempered by G1's prior analysis (lag-only, terminal value always correct). R9-13 extends C6 with new `def` locals. R9-14 collects judgment calls: the `fetchZwave/ZigbeeDataForCheckpoint` twins, the `_schemaVersion` sentinel (keep only if on-disk persistence is planned — cross-ref A9), the likely-dead `renderComparison` manual-build branch (**verify the hub payload before removing**), `getAppTypeId`'s raw `httpGet` (cross-ref C10), and `fullJsonCb` checking `resp?.status` without `resp.hasError()` first (fail-soft).
