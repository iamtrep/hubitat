...
Some methods have large blast radius and must not gain new fetches casually:

- `buildSharedCache()`
- `getStructuredAlerts()`
- `apiLive()`
- top-level dashboard/health aggregation paths

Any new fetch added there must satisfy all of these:

- broad enough value to justify running on a common path
...
2. normalize/shape data in Groovy
