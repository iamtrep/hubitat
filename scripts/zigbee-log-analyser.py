#!/usr/bin/env python3
"""
Zigbee Log Analyzer (Hubitat) — Plus
====================================

Features
- Parses Hubitat websocket JSON lines and text log lines.
- ZCL frame parsing: frame control, manufacturer-specific bit, direction, disable-default-response.
- Two-pass analysis: builds DNI->manufacturer index; **first pass also learns manufacturer from OTA client requests**.
- Filters: manufacturer code, device id, device name, DNI (hex), clusters, unsolicited-only.
- ZDO (profile 0x0000): optionally include network/discovery traffic.
- ZCL Global Commands: Read/Write/Configure/Report/Discover counts.
- OTA (0x0019) inbound-only sniff: extracts manufacturer/image type/file version from client requests; tracks block offsets and upgrade end.
- Outputs: table or CSV; optional filtered raw log output.
- Heatmaps: Top talkers and continuous RSSI (forward-fill).
- Manufacturer names: built-in stub + optional external JSON + **full list block at the bottom** you can paste.

Usage examples
--------------
python3 zigbee-log-analyzer-plus.py cpz.log --unsolicited-only --ota-details
...
python3 zigbee-log-analyzer-plus.py cpz.log --manufacturer-db manufacturers.json
"""
...
# Minimal stub; will be merged with external JSON and/or full list at the bottom
KNOWN_MANUFACTURERS = {
...
# ---------------------------------------------
# FULL MANUFACTURER MAP (paste your complete table below)
# ---------------------------------------------
# Note: Codes normalized to 4-hex uppercase (e.g., '119C'). Special codes:
#  - 'FFFF': All / Match-All (OTA header may use this)
#  - '0000': Unknown
# The analyzer normalizes inputs and falls back to OTA request manufacturer when needed.
...
try:
    ...
    print('[WARNING] Could not merge full manufacturer table:', e)
...
def parse_zcl(payload_hex: List[str]) -> dict:
    """Decode ZCL header: FC, optional manufacturer, SEQ, CMD."""
    res = {
        'fc': None,
        'frame_type': None,
...
def parse_ota_request_payload(payload_hex: List[str], zcl_header: dict) -> Optional[dict]:
    """Parse OTA client->server request payloads (0x01, 0x03, 0x06)."""
    try:
...
# ---------------------------------------------
# Manufacturer DB loading, merged with stub
# ---------------------------------------------

def load_manufacturer_db(path: Optional[str]) -> Dict[str, str]:
    db = {normalize_mfr_code(k): v for k, v in KNOWN_MANUFACTURERS.items()}
    if path:
        try:
            with open(path, 'r') as f:
                ext = json.load(f)
            for k, v in ext.items():
                db[normalize_mfr_code(k)] = v
        except Exception as e:
            print(f"[WARNING] Could not load manufacturer DB '{path}': {e}", file=sys.stderr)
    return db

# ---------------------------------------------
# First pass: build DNI -> manufacturer code (header + OTA sniff)
# ---------------------------------------------
...
def build_dni_mfr_index(filepath: str) -> Dict[int, str]:
    ...
    with open(filepath, 'r') as f:
        for line in f:
            ...
            z = parse_zcl(pay)
            dni = rec['network_id']
            # 1) Header manufacturer (manuf-specific ZCL)
            if z['manufacturer_code'] and dni is not None:
                index[dni] = z['manufacturer_code']
                continue
            # 2) OTA client requests (inbound-only logs)
    ...
    with open(filepath, 'r') as f:
        for line in f:
            ...
            if mfr_filter:
                ...
                if not mfr_this or normalize_mfr_code(mfr_this) != normalize_mfr_code(mfr_filter):
                    continue
            ...
            # unsolicited rule; always keep OTA cluster
...
def summarize_missing_manufacturers(devices: dict, manufacturer_db: Dict[str, str]):
    missing = Counter()
    for _, info in devices.items():
        mfr_code = normalize_mfr_code(info.get('mfr_code') or info.get('ota_req_mfr'))
        if mfr_code and mfr_code not in manufacturer_db:
            missing[mfr_code] += 1
    if missing:
        print("\nMissing manufacturer codes (consider adding to --manufacturer-db or the full list block):")
        for code, n in missing.most_common():
            print(f"  {code}  seen {n} time(s)")
...
def print_table(devices: dict, time_range: Tuple[Optional[str], Optional[str]], filters: dict, manufacturer_db: Dict[str, str], global_summary: dict):
    ...
    if filters.get('manufacturer'):
        parts.append(f"manufacturer={filters['manufacturer']}")
    if filters.get('dni'):
    ...
    for name, info in sorted(devices.items(), key=lambda kv: kv[1]['count'], reverse=True):
        ...
        mfr_code = normalize_mfr_code(info['mfr_code'] or info.get('ota_req_mfr'))
        mfr_name = manufacturer_db.get(mfr_code, '')
        if not mfr_name:
            if mfr_code == 'FFFF':
                mfr_name = 'All / Match-All'
            elif mfr_code in ('0000', ''):
                mfr_name = 'Unknown'
    ...
    summarize_missing_manufacturers(devices, manufacturer_db)
...
# ---------------------------------------------
# Heatmaps / Top talkers (forward-fill for RSSI)
# ---------------------------------------------
...
def main():
    ...
    dni_filter = None
    if args.dni:
        dni_filter = int(args.dni.upper().replace('0X', ''), 16)
