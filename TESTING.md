...
**Late-firing observations — assert on events, not logs.** `LogCapture` can intermittently *miss* a log line that fires more than ~5 s after the action (a scheduled `runIn` callback, emergency timer, or debounce), especially across several sequential `LogCapture`/`EventCapture` context pairs in one run — a hub-side per-connection `/logsocket` quirk that accumulates across contexts, while `EventCapture` (`/eventsocket`) stays reliable through the whole run. So for any assertion on a late observation, assert on the resulting device-state event (`assert_events`) rather than the log line; the capability-event shape is stabler than log wording anyway. If a late `assert_logs` is genuinely unavoidable, run that case in isolation or move it earlier in the list to shrink the interference window.
...
Most drivers in this repo need no automated tests. Four options when one is warranted.

However a driver is exercised, there are **two harness mechanisms** for driving it — orthogonal to the four options below:

- **Maker API (black-box)** — create a standalone device of the driver, expose it via a dedicated Maker API instance, then call commands (`GET /apps/api/{app}/devices/{id}/{command}`) and read attributes (`GET /apps/api/{app}/devices/{id}`) from outside. Reaches only declared commands and published attributes, not `parse()` or internal helpers. Production-representative invocation path; prefer it when the command/attribute surface is what's under test.
- **Child-device test app (white-box)** — a parent test app creates the driver-under-test as a *child* device and can call *any* public method on it (including `parse(String description)` and other non-command entry points) plus `updateSetting()` to configure its preferences. The only way to inject protocol fixtures or reach paths not exposed as commands; the `TestFixtureManager` option below builds on it.
